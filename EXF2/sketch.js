function sketch(p) {
    ...
    function resetAdjustments() {
        cellSizeSlider.value(12);
        emptyThresholdSlider.value(25);
        fThresholdSlider.value(50);
        eThresholdSlider.value(75);
    ...
    function generateAscii() {
        ...
        for (let y = imgDrawParams.y; y < imgDrawParams.y + imgDrawParams.height; y += cellHeight) {
            for (let x = imgDrawParams.x; x < imgDrawParams.x + imgDrawParams.width; x += cellWidth) {
                ...
                p.text(charToDraw, pixelX, pixelY);
            }
        }
    }
    
    // --- Вспомогательные функции и экспорт ---
    ...
    function exportPNG() {
        ...
        if (showOriginal) {
            exportPG.image(originalImg, 0, 0, exportPG.width, exportPG.height);
        }
        ...
        for (let y = 0; y < exportHeight; y += scaledCellHeight) {
            for (let x = 0; x < exportWidth; x += scaledCellWidth) {
                const unscaledX = x / scaleFactor;
                const unscaledY = y / scaleFactor;
                ...
                const brightness = p.brightness(c);
                const charToDraw = getAsciiChar(brightness);
                exportPG.text(charToDraw, x + scaledCellWidth / 2, y + scaledCellHeight / 2);
            }
        }
    ...
    function exportSVG() {
        ...
        link.href = url;
        link.download = 'exf-studio-art.svg';
